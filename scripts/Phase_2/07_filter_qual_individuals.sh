#!/usr/bin/env bash
# 07_filter_qual_individuals.sh — Filter variants by quality (SNPs QUAL>=100, INDELs QUAL>=150)
# Input:  IND_VCF_DIR/{species}.vcf.gz  (from 06_call_individuals_VCF)
...
# Usage: bash 07_filter_qual_individuals.sh

set -euo pipefail
shopt -s nullglob
source "$(dirname "$0")/config.sh"

IND_VCF_DIR="${PHASE2_DIR}/results_refdb/INDIV_species_vcf_con_INDELS"
IND_VCF_FILT_DIR="${PHASE2_DIR}/results_refdb/INDIV_species_vcf_FINAL_QUAL_100-150"
