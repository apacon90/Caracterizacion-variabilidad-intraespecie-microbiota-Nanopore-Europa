...
# Usage:  bash 12_mapp_call.sh

set -euo pipefail
source "$(dirname "$0")/config.sh"

REF_DIR="${RESULTS_DIR}/ref_per_species"
OUT_BAM="${RESULTS_DIR}/pool_mappings"
SELECT="${RESULTS_DIR}/select_sp.tsv"

mkdir -p "$REF_DIR" "$OUT_BAM"
...
echo "Done. BAM files saved in: $OUT_BAM"
