...
# 12_pool_by_population.sh — Split reads into N pools of fixed size per species and country
# Input:  SPECIES_READS_DIR/{country}/{species}.reads.fa  (from 06_reads_species.sh)
...
# Usage: bash 12_pool_by_population.sh
...
find "$POOLS_DIR" \( -name "*_pool*.fa" -o -name "*_pool*.bam" -o -name "*_pool*.bam.bai" \) -delete 2>/dev/null || true
mkdir -p "$POOLS_DIR"

find "$SPECIES_READS_DIR" -mindepth 2 -maxdepth 2 -type f -name "*.reads.fa" | sort | while read -r INFA; do
    COUNTRY_DIR=$(basename "$(dirname "$INFA")")
    COUNTRY="${COUNTRY_DIR%_dataset}"
