...
# 16_export_variants.sh — Export filtered variants to TSV with genotypes per pool
# Input:  VCF_FILTERED_DIR/{species}.FINAL_pool3.vcf.gz  (from 15_filter_pools.sh)
# Output: RESULTS_DIR/ALL_species_pool3_variants_with_pools.tsv
#
# Usage: bash 16_export_variants.sh

set -euo pipefail
source "$(dirname "$0")/../config.sh"

OUT="${RESULTS_DIR}/ALL_species_pool3_variants_with_pools.tsv"
