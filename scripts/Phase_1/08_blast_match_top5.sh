#!/usr/bin/env bash
# 08_blast_match_top5.sh — BLASTn top5 hits for each assembled consensus vs RefSeq207nr
# Input:  RRN_CONSENSUS_DIR/{species}/assembly_reoriented.fasta  (from 07_B)
# Output: RESULTS_DIR/blast_top5_taxrep.tsv
#
# Builds BLAST DB if missing, then runs megablast (top5, evalue<1e-10).
# Usage: bash 08_blast_match_top5.sh
